...
if __name__ == '__main__':
    ...
    print ("\n\nDownloading Components Report CSV file ..")
    compOutName = fileName + "_components.csv"
    opUrl1 =  "https://" + protecodeHost + "/api/app/" + str(id) + "/csv-libs"
    ...
    try:
        scanOp1 = subprocess.check_output(cmd, shell=True)
            ...
            output = e.output
            returncode = e.returncode
            print(" Error while extracting component CSV file:{} \n ReturnCode:{}".format(output, returncode))
            sys.exit(-1)

    # vulnerability.csv extract
    print ("\nDownloading Vulnerabilities Report CSV file ..")
    vulnOutName = fileName + "_vulnerabilities.csv"
    opUrl2 =  "https://" + protecodeHost + "/api/app/" + str(id) + "/csv-vulns"
    cmd = "curl -u " + protecodeLoginAndPass + " " + opUrl2 + " > " + vulnOutName
    print (cmd)
    try:
        scanOp2 = subprocess.check_output(cmd, shell=True)
    except subprocess.CalledProcessError as e:
            output = e.output
            returncode = e.returncode
            print(" Error while extracting vulnerabilities CSV file:{} \n ReturnCode:{}".format(output, returncode))
            sys.exit(-1)
    print ("\n \n")
    ...
    #----------------------------------------------------
    #Process CSV Reports. Extract JSON Output for Import
    #----------------------------------------------------
    print ("\n\nProcessing Vulnerabilities and Component CSV reports...")
    scriptDir = os.path.dirname(sys.argv[0])#os.path.realpath(__file__)
    print ("Script Path:{}".format(os.path.abspath(scriptDir)))

    # Read Vulnerabilities CSV report
    csvFileloc1 = os.path.join(scriptDir, compOutName)
    with open(csvFileloc1) as csvFile1:
        csvRead1 = csv.reader(csvFile1)
        componentData = list(csvRead1)

    # Read Components CSV report
    csvFileloc2 = os.path.join(scriptDir, vulnOutName)
    with open(csvFileloc2) as csvFile2:
        csvRead2 = csv.reader(csvFile2)
        vulnData = list(csvRead2)

    rowCnt = sum(1 for row in vulnData)


    #Exit if zero Vulnerabilities detected.
    if (rowCnt < 2):
        print("No Vulnerability Detected. Exiting...")
        #sys.exit(-1)
    else:
        print("Vulnerabilities Detected!!! Fix it now!!!...")
        #sys.exit(-1)
